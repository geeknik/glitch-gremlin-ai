...
MAIN_PROGRAM=$(solana-keygen grind --starts-with glch1:1 --num-threads 4)
echo "Main program ID: $MAIN_PROGRAM"

# Generate governance program ID
GOVERNANCE_PROGRAM=$(solana-keygen grind --starts-with glch2:1 --num-threads 4)
echo "Governance program ID: $GOVERNANCE_PROGRAM"

# Generate token program ID
TOKEN_PROGRAM=$(solana-keygen grind --starts-with glch3:1 --num-threads 4)
echo "Token program ID: $TOKEN_PROGRAM"

# Generate worker program ID
WORKER_PROGRAM=$(solana-keygen grind --starts-with glch4:1 --num-threads 4)
