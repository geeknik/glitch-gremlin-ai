...
impl GovernanceConfig {
    pub fn validate(&self) -> Result<()> {
        ...
        require!(
            self.proposal_rate_limit > 0,
            GovernanceError::InvalidRateLimit
        );
        require!(
            self.min_stake_amount > 0,
            GovernanceError::InvalidMinStake
        );
        require!(
            self.min_voting_period > 0 && self.min_voting_period < self.max_voting_period,
...
impl GovernanceState {
    pub fn check_proposal_rate_limit(&self, current_time: i64) -> Result<()> {
        if current_time - self.last_proposal_time <= self.config.proposal_rate_window 
            && self.proposal_count_window >= self.config.proposal_rate_limit {
    ...
    pub fn verify_gremlinai_token(token_mint: &Pubkey) -> Result<()> {
        require!(
            token_mint == &Pubkey::from_str(GREMLINAI_TOKEN_MINT).unwrap(),
    ...
    pub fn verify_dev_authority(authority: &Pubkey) -> Result<()> {
        require!(
            authority == &Pubkey::from_str(DEV_WALLET).unwrap(),
...
pub fn validate_token_mint(token_mint: &Pubkey) -> bool {
    token_mint == &Pubkey::from_str(GREMLINAI_TOKEN_MINT).unwrap()
}

pub fn validate_authority(authority: &Pubkey) -> bool {
    authority == &Pubkey::from_str(DEV_WALLET).unwrap()
}
